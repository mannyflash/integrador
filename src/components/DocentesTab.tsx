...
export function DocentesTab({
  ...
  const toggleSeleccionarTodos = () => {
    if (seleccionados.length === docentes.length) {
      setSeleccionados([])
    } else {
      setSeleccionados(docentes.map((d) => d.Matricula))
    }
  }

  const toggleSeleccionarDocente = (matricula: string) => {
    setSeleccionados((prev) => (prev.includes(matricula) ? prev.filter((m) => m !== matricula) : [...prev, matricula]))
  }

  const eliminarSeleccionados = async () => {
    if (seleccionados.length === 0) {
      Swal.fire("Advertencia", "No has seleccionado ningún docente", "warning")
      return
    }

    const result = await Swal.fire({
      title: "¿Estás seguro?",
      text: `Se eliminarán ${seleccionados.length} docente(s). No podrás revertir esta acción!`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
    ...
    if (result.isConfirmed) {
      try {
        await Promise.all(seleccionados.map((matricula) => deleteDoc(doc(db, "Docentes", matricula))))
        Swal.fire("Eliminados!", `${seleccionados.length} docente(s) eliminado(s).`, "success")
        setSeleccionados([])
        cargarDocentes()
      } catch (error) {
        console.error("Error al eliminar docentes:", error)
        Swal.fire("Error!", "Ha ocurrido un error al eliminar los docentes.", "error")
      }
    }
  }
              ...
              {seleccionados.length > 0 && (
                <div className="sticky top-0 z-20 bg-white dark:bg-gray-800 p-3 border-b flex items-center justify-between">
                  <span className={`text-sm font-medium ${isDarkMode ? "text-white" : "text-gray-700"}`}>
                    {seleccionados.length} docente(s) seleccionado(s)
                  </span>
                  ...
                  {docentesFiltrados.map((docente) => (
                    ...
                    >
                      <TableCell>
                        <input
                          type="checkbox"
                          checked={seleccionados.includes(docente.Matricula)}
                          onChange={() => toggleSeleccionarDocente(docente.Matricula)}
                          className="h-4 w-4 rounded border-gray-300 cursor-pointer"
                        />
                      </TableCell>
