...
export function LaboratoristasTab({
  ...
  const toggleSeleccionarTodos = () => {
    if (seleccionados.length === laboratoristasFiltrados.length) {
      setSeleccionados([])
    } else {
      setSeleccionados(laboratoristasFiltrados.map((l) => l.Matricula))
    }
  }

  const toggleSeleccionarLaboratorista = (matricula: string) => {
    setSeleccionados((prev) => (prev.includes(matricula) ? prev.filter((m) => m !== matricula) : [...prev, matricula]))
  }

  const eliminarSeleccionados = async () => {
    if (seleccionados.length === 0) {
      Swal.fire("Advertencia", "No has seleccionado ningún laboratorista", "warning")
      return
    }

    const result = await Swal.fire({
      title: "¿Estás seguro?",
      text: `Se eliminarán ${seleccionados.length} laboratorista(s). No podrás revertir esta acción!`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
    ...
    if (result.isConfirmed) {
      try {
        await Promise.all(seleccionados.map((matricula) => deleteDoc(doc(db, "Laboratoristas", matricula))))
        Swal.fire("Eliminados!", `${seleccionados.length} laboratorista(s) eliminado(s).`, "success")
        setSeleccionados([])
        cargarLaboratoristas()
      } catch (error) {
        console.error("Error al eliminar laboratoristas:", error)
        Swal.fire("Error!", "Ha ocurrido un error al eliminar los laboratoristas.", "error")
      }
    }
  }
                ...
                {seleccionados.length > 0 && (
                  <div className="sticky top-0 z-20 bg-white dark:bg-gray-800 p-3 border-b flex items-center justify-between mb-4">
                    <span className={`text-sm font-medium ${textColor}`}>
                      {seleccionados.length} laboratorista(s) seleccionado(s)
                    </span>
                ...
                <Table>
                  <TableHeader className="sticky top-0 z-10">
                    <TableRow className={isDarkMode ? "bg-[#1d5631]/20" : "bg-[#952952]/5"}>
                      <TableHead className={`${textColor} font-medium`}>
                        <input
                          type="checkbox"
                          checked={
                            seleccionados.length === laboratoristasFiltrados.length &&
                            laboratoristasFiltrados.length > 0
                          }
                    ...
                    {laboratoristasFiltrados.map((laboratorista) => (
                      ...
                      >
                        <TableCell>
                          <input
                            type="checkbox"
                            checked={seleccionados.includes(laboratorista.Matricula)}
                            onChange={() => toggleSeleccionarLaboratorista(laboratorista.Matricula)}
                            className="h-4 w-4 rounded border-gray-300 cursor-pointer"
                          />
                        </TableCell>
